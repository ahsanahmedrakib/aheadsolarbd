@php
    ...
    $showWhatsapp = $tgl('show-whatsapp');
    $waNum = preg_replace('/[^0-9]/', '', $fld('whatsapp-number') ?? '');
    $showMessenger = $tgl('show-messenger');
    $msgUser = $fld('messenger-username') ?? '';
    $showFacebook = $tgl('show-facebook');
    $fbUrl = $fld('facebook-url') ?? '';
    $showLinkedin = $tgl('show-linkedin');
    $liUrl = $fld('linkedin-url') ?? '';
    $showYoutube = $tgl('show-youtube');
    $ytUrl = $fld('youtube-url') ?? '';
    ...
    if ($showWhatsapp && $waNum) {
        ...
        $links[] = [
            ...
            'aria' => 'WhatsApp',
            'bg' => '#25D366',
            'href' => 'https://wa.me/' . $waNum . '?text=' . urlencode($msg),
            'icon' => 'whatsapp',
        ];
    }
