...
<div class="bg-white font-sans overflow-x-hidden">
    <section class="bg-white py-20 lg:py-25 px-4 sm:px-6 lg:px-8">
        <div class="solar-container space-y-12">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch">
                @foreach ($services as $i => $service)
                    <div class="reveal h-115" data-variant="fade-up" data-delay="{{ ($i % 3) * 120 }}">
                        <a href="{{ url('services/' . $service->slug) }}" class="relative h-full rounded-lg overflow-hidden shadow-sm group flex flex-col justify-end p-4 transition-transform duration-300 hover:-translate-y-1">
