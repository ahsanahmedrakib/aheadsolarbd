    ...
    <div class="solar-container">
        <div class="flex flex-col items-center text-center max-w-3xl mx-auto mb-16">
            <div class="reveal" data-variant="fade-up">
                <span class="section-eyebrow">Choosing The Right Model</span>
            </div>

            <div class="reveal" data-variant="fade-up" data-delay="100">
                <h2 class="mt-4 font-heading text-3xl sm:text-4xl lg:text-[52px] font-bold text-accent-500 tracking-tight leading-[1.1]">Selecting Appropriate <span class="text-accent-500">Financial Model</span></h2>
            </div>

            <div class="reveal" data-variant="fade-up" data-delay="180">
                <p class="mt-4 text-[#888888] text-sm sm:text-base font-normal leading-relaxed">Compare the CapEx, OpEx, and BOT solar models side by side to choose the financial structure that best fits your business goals.</p>
            </div>
        </div>

        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            @foreach ($columns as $i => $col)
                <div class="reveal h-full" data-variant="fade-up" data-delay="{{ $i * 120 }}">
                    <div class="group bg-secondary rounded-lg p-8 sm:p-10 h-full shadow-sm border border-white/60 hover:shadow-xl hover:-translate-y-1.5 transition-all duration-500 relative overflow-hidden card-shine flex flex-col">
                        <div class="flex items-center justify-between mb-8">
                            <div class="w-14 h-14 rounded-[18px] text-white bg-accent-500 flex items-center justify-center shadow-sm transition-all duration-500 group-hover:scale-110 group-hover:rotate-6">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-6 h-6"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $col['d'] }}"/></svg>
                            </div>
                            <span class="font-heading text-2xl sm:text-3xl font-extrabold text-accent-500 tracking-wider">{{ $col['label'] }}</span>
                        </div>

                        <h3 class="font-heading text-xl sm:text-2xl font-bold text-accent-500 tracking-tight mb-6">{{ $col['title'] }}</h3>

                        <ul class="space-y-4">
                            @foreach ($col['items'] as $item)
                                <li class="flex items-start gap-3">
                                    <span class="shrink-0 w-5 h-5 rounded-full bg-accent-500 text-white flex items-center justify-center mt-0.5">
                                    ...
                                    <span class="text-sm sm:text-base text-[#888888] font-medium leading-relaxed">{{ $item }}</span>
                                </li>
                            @endforeach
                        </ul>
                    </div>
