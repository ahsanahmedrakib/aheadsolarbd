                    ...
                    <div class="bg-secondary rounded-lg overflow-hidden border border-white shadow-sm">
                        ...
                        <nav class="flex flex-col">
                            @foreach ($allProjects as $p)
                                <a href="{{ url('projects/' . $p->slug) }}" class="flex items-center justify-between px-5 py-3.5 text-xs font-bold border-b border-forest-700/10 last:border-0 text-left transition-colors {{ $p->slug === $project->slug ? 'bg-accent-500 text-white' : 'text-accent-500 hover:bg-white' }}">
                                    <span>{{ $p->title }}</span>
            ...
            <main class="w-full lg:w-[70%] flex flex-col gap-12 lg:pl-4">
                <div class="reveal" data-variant="fade-up">
                    <section class="flex flex-col gap-6">
                        ...
                        <div class="text-[#888888] text-sm leading-relaxed space-y-4">
